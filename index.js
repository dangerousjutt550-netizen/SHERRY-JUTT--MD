...
// Owner numbers
const ownerNumber = ["923139741395"];
...
function loadLocalFiles() {
  ...
  if (fs.existsSync('./lib/arslan.html')) {
    console.log("✅ sherryjutt.html found in lib folder");
  } else {
    console.log("⚠️ sherry.html not found in lib folder");
  }
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("SHERRY JUTT-MD~")) {
    console.log("❌ Invalid SHERRY JUTT-MD session format");
    process.exit(1);
  }
  ...
  console.log("♻️ SHERRYJUTT-MD session restored successfully");
}
...
// Serve HTML page
app.get('/', (req, res) => {
  const htmlPath = path.join(__dirname, 'lib', 'SHERRY.html');
  if (fs.existsSync(htmlPath)) {
    ...
    res.send(`
      ...
      <html>
      <head>
        <title>SHERRY JUTT- MD Bot</title>
        <style>
...
app.get('/lib/SHERRY.html', (req, res) => {
  res.redirect('/');
});
...
async function connectToWA() {
  ...
  console.log(`🤖 K
  SHERRY JUTT-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ SHERRYJUTT-MD Bot connected to WhatsApp!");
      ...
      try {
        ...
        sock.sendMessage(ownerNumber[0] + '923139741395@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
